...
**Answer:**
A closure is when a function "remembers" the variables from its outer lexical scope even after that outer function has finished executing. JavaScript functions form closures over their surrounding scope at the point they're defined, not called. This is what makes patterns like private variables, counters, and function factories possible.
...
**Answer:**
`===` checks value and type with no coercion — it's strict equality. `==` performs type coercion before comparing, which leads to surprising results like `'' == 0` being `true`. In production code, `===` is the default choice; `==` is mainly useful for deliberately checking `null`/`undefined` together.
